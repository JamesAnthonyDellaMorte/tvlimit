    ...
    let mut plug: SmartPlug = match config.clone() {
        Ok(c) => {
            ...
            smart_plug::SmartPlug::new(
                format!("{}:9999", c.plug_ip),
                format!("{}:8060", c.roku_ip),
            )
        }
...
fn wait_till_1230() {
    ...
    match target_time {
        Some(target_time) => {
            ...
            let mut sleep_duration = match sleep_delta.to_std() {
                Ok(d) => d,
                Err(_) => {
            ...
            if now.hour() <= 12 && now.minute() < 30 {
                sleep_duration += time::Duration::from_secs(86400);
                println!("Looks like some tv watching is happening before 12:30 pm!");
            }
            println!(
                "We are waiting until 12:30 pm to turn on TV! There are {} left.",
...
fn run_loop(p: &mut smart_plug::SmartPlug, ttw: Option<String>) {
    ...
    let mut timer = parse_duration(
        &fs::read_to_string("tvtimer.txt")
            .unwrap_or("0\n".to_owned())
            .trim()
            .parse::<String>()
        ...
        match p.state {
            smart_plug::TVState::Play => {
                if (timer.as_secs() % 30) == 0 {
                    ...
                    fs::write("tvtimer.txt", format_duration(timer).to_string()).unwrap_or(());
                }
            ...
            smart_plug::TVState::Pause => {
                println!(
                    "Looks like something on {} is paused, timer is at {}",
                    p.whats_playing,
                    format_duration(timer)
                );
            ...
            smart_plug::TVState::Idle => {
                println!(
                    "No media is playing via a channel, timer is at {}",
                    format_duration(timer)
                );
